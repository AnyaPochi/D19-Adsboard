...
def respond_to_me(request):
    queryset = Respond.objects.filter(post__in=Post.objects.filter(author_id=request.user))
    ...
    if request.method == "POST":
        ...
        if accept:
            ...
            Respond.objects.get(pk=id).delete()
...
def respond_create(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Respond.objects.create(**form.cleaned_data)
